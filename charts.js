...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var metArray = data.metadata.filter(sampleObj => sampleObj.id == sample);

    // Deliverable 1: 5. Create a variable that holds the first sample in the array.

    var firstSamp = sampArray[0];

    // Deliverable 3: 2. Create a variable that holds the first sample in the metadata array.

    var firstMet = metArray[0]; 

    // Deliverable 1: 6. Create variables that hold the otu_ids, otu_labels, and sample_values.

    var otuIds = firstSamp.otu_ids;
    var otuLabels = firstSamp.otu_labels;
    var sampleValues = firstSamp.sample_values;

    // Deliverable 3: 3. Create a variable that holds the washing frequency.

    var wFreq = parseFloat(firstMet.wfreq);
    ...
    var barLayout = {
      ...
      xaxis: {title: "Bacterial Sample Values"},
      // yaxis: {title: "otuIDs"}
    };
    ...
    var bubbleData = [{
      ...
      text: otuLabels,
      mode: "markers",
      marker: {color: otuIds, size: sampleValues, colorscale: 'Jet'}
    }];
